...
#and to provide common methods that operate on the list. For example you can implement
#an `append` method that adds a value to the end of the list.
#Note that if we're only tracking the head of the list, this runs in linear time - $O(N)$ -
#since you have to iterate through the entire list to get to the tail node. However, prepending (adding to the head of the list) can be done in constant $O(1)$ time. You'll implement this `prepend` method in the Linked List Practice notebook.
...
# <span class="graffiti-highlight graffiti-id_eaeyzao-id_kkwn0on"><i></i><button>Show Solution</button></span>
